...
bool ImGui::ButtonEx(const char* label, const ImVec2 & size_arg, ImGuiButtonFlags flags)
{
    ...
    auto borderColor = ImColor(50, 49, 55, 255);
    auto topColor = ImColor(30, 35, 36, 255);
    auto bottomColor = ImColor(16, 20, 24, 255);
    auto topColorHovered = ImColor(30, 35, 36, 255);
    auto bottomColorHovered = ImColor(26, 30, 34, 255);
    auto heldColorTop = ImColor(40, 45, 46, 255);
    auto heldColorBottom = ImColor(36, 40, 44, 255);
    ...
    if (hovered) {
        if (held)
        {
            window->DrawList->AddRectFilledMultiColor(bb.Min - ImVec2(0, 1), bb.Max, heldColorTop, heldColorTop, heldColorBottom, heldColorBottom);
        }
        else {
            window->DrawList->AddRectFilledMultiColor(bb.Min - ImVec2(0, 1), bb.Max, topColorHovered, topColorHovered, bottomColorHovered, bottomColorHovered);
        }
    }
    else {
        window->DrawList->AddRectFilledMultiColor(bb.Min - ImVec2(0, 1), bb.Max, topColor, topColor, bottomColor, bottomColor);
    }
    window->DrawList->AddRect(bb.Min - ImVec2(0, 1), bb.Max, borderColor, 0, false, 1);
...
/*void ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale)
{
    ...
    g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b, center + c, ImGui::GetColorU32(ImGuiCol_Text));
}
*/
    ...
    if (g.ActiveId == id) {
        for (auto i = 0; i < 5; i++) {
            if (io->MouseDown[i]) {
                switch (i) {
                case 0:
                    key = 107; // ButtonCode_t::MOUSE_LEFT
                    break;
                case 1:
                    key = 108;
                    break;
