...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2(Globals::screenWidth - NOTIFY_PADDING_X, Globals::screenHeight - NOTIFY_PADDING_Y - height), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos(Globals::screenWidth / 3.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
