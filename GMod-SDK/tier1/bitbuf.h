...
namespace bitbuf
{
	...
	inline uint32_t ZigZagEncode32(int32 n)
	{
		// Note:  the right-shift must be arithmetic
		return(n << 1) ^ (n >> 31);
	}

	inline int32 ZigZagDecode32(uint32_t n)
	{
		return(n >> 1) ^ -static_cast<int32>(n & 1);
	}

	inline uint64_t ZigZagEncode64(int64 n)
	{
		// Note:  the right-shift must be arithmetic
		return(n << 1) ^ (n >> 63);
	}

	inline int64 ZigZagDecode64(uint64_t n)
	{
